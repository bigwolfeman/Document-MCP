...
async def delete_note(path: str, auth: AuthContext = Depends(get_auth_context)):
    """Delete a note."""
    user_id = auth.user_id
    _ensure_write_allowed(user_id)
    vault_service = VaultService()
    ...
    try:
        ...
        try:
            with conn:
                ...
                conn.execute("DELETE FROM note_links WHERE user_id = ? AND target_path = ?", (user_id, note_path))
